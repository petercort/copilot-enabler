...
5. **Views** (`src/views/*`): `DashboardPanel` + `SettingsPanel` (webviews with inline HTML/CSS), `FeatureTreeProvider` / `RecommendationTreeProvider`, and `StatusBarManager`. Views are pure consumers of `AnalysisResult`.
6. **Static optimization scans** (`src/core/optimization/*`) retain the reusable token- and instruction-quality findings used by the dashboard scorecard, without coupling them to the removed Promptimizer subsystem.
